...
export default {
  ...
  computed: {
    houseDateFormatted () {
      ...
      return `The last House session was ${moment(this.houseDate)} ${moment(this.houseDate).format('dddd, MMMM Do')}`
    },
    senateDateFormatted () {
      if (this.senateInSession) {
        return `The Senate was in session today.`
      }
      return `The last Senate session was ${moment(this.senateDate)} ${moment(this.senateDate).format('dddd, MMMM Do')}`
    }
  }
}
</script>
